...
-- alter tables (ignorable if columns exist)
ALTER TABLE ai_table_schema ADD COLUMN allow_query TINYINT DEFAULT 0 COMMENT '允许AI查询';
ALTER TABLE ai_chat_log ADD COLUMN query_mode VARCHAR(50) DEFAULT NULL;
ALTER TABLE ai_chat_log ADD COLUMN free_sql_reason TEXT NULL;
ALTER TABLE ai_chat_log ADD COLUMN used_tables JSON NULL;
ALTER TABLE ai_chat_log ADD COLUMN risk_level VARCHAR(50) DEFAULT NULL;
ALTER TABLE ai_chat_log ADD COLUMN estimated_rows BIGINT DEFAULT NULL;

UPDATE ai_table_schema SET allow_query = 1 WHERE table_name IN (
    'ad_factory_info','ad_product_info','ad_product_parts',
    'ad_product_record','ad_raw_record','ad_month_inventory',
    'ad_order_info','ad_order_item'
);

INSERT INTO sys_config (config_name, config_key, config_value, config_type, create_by, create_time, remark) VALUES
('AI Agent开关','ai.agent.enabled','true','Y','admin',NOW(),''),
('AI Agent最大步数','ai.agent.max_steps','5','Y','admin',NOW(),''),
('AI Agent默认LIMIT','ai.agent.default_limit','100','Y','admin',NOW(),''),
('AI展示查询计划','ai.agent.show_plan','true','Y','admin',NOW(),''),
('AI展示步骤SQL','ai.agent.show_step_sql','true','Y','admin',NOW(),''),
('AI允许追加步骤','ai.agent.allow_followup_steps','true','Y','admin',NOW(),''),
('AI自由SQL开关','ai.free_sql.enabled','true','Y','admin',NOW(),''),
('AI自由SQL确认','ai.free_sql.require_confirm','false','Y','admin',NOW(),''),
('AI自由SQL最大行','ai.free_sql.max_rows','200','Y','admin',NOW(),''),
('AI自由SQLEXPLAIN','ai.free_sql.explain_before_run','true','Y','admin',NOW(),''),
('AI自由SQL最大预估','ai.free_sql.max_estimated_rows','50000','Y','admin',NOW(),''),
('AI SQL超时','ai.sql.timeout_seconds','10','Y','admin',NOW(),''),
('AI默认展示SQL','ai.sql.show_sql_default','true','Y','admin',NOW(),'')
ON DUPLICATE KEY UPDATE config_name=VALUES(config_name), config_value=VALUES(config_value), remark=VALUES(remark), update_time=NOW();
